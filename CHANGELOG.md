...
- Unit tests covering `/nac config` argument handling in `ConfigCommand` and the single-quote parsing in `ArgumentParser` that reads deny messages. They pin current behaviour, including an apostrophe inside a quoted message ending it early (#60). `COMMANDS.md` and `USER_GUIDE.md` now say what those tests show: the two deny-message options require single quotes even around one word, and no other option reads them.
...
- A `/nac` sub-command refused for a missing permission no longer reports itself to the server as having been used incorrectly. Each of the five permission gates returned `false` from `onCommand` after sending the denial, which is the value that asks the server to print the command's usage string. Nothing was printed in practice, because `plugin.yml` declares no `usage` key for the command; adding one would have followed every denial with usage text, showing the command's syntax to exactly the sender the permission node exists to keep away from it. The gates now return `true`, as a refusal is a correct use of the command that was answered with no.
