...
- Unit tests covering `/nac config` argument handling in `ConfigCommand` and the single-quote parsing in `ArgumentParser` that reads deny messages. They pin current behaviour, including an apostrophe inside a quoted message ending it early (#60).
